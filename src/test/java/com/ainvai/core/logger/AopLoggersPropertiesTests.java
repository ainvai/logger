...
class AopLoggersPropertiesTests {
    ...
    runner.run(
        (context) -> {
          ...
          assertThat(properties.getEnteringMessage())
              .isEqualTo("Entering method={method} with parameters=[{parameters}]");
        });
  }
    ...
    runner.run(
        (context) -> {
          ...
          assertThat(properties.getExitedMessage())
              .isEqualTo("method={method} msg=\"exited normally with return value\" result=[{return-value}]");
        });
  }
    ...
    runner.run(
        (context) -> {
          ...
          assertThat(properties.getExitedAbnormallyMessage())
              .isEqualTo("method={method} exited abnormally with exception=[{exception}]");
        });
  }
  ...
  void elapsedMessage_defaultValue() {
    runner.run(
        (context) -> {
          final AopLoggersProperties properties = context.getBean(AopLoggersProperties.class);
          assertThat(properties.getElapsedMessage()).isEqualTo("method={method} elapsed={elapsed}");
        });
  }
    ...
    runner.run(
        (context) -> {
          ...
          assertThat(properties.getElapsedWarningMessage())
              .isEqualTo("method={method} reached elapsedTimeLimit=[{elapsed-time-limit}]");
        });
  }
